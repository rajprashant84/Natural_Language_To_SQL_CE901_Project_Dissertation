...
class SQL_Query:
    ...
    def from_tokenized_dict(cls, d):
        ...
        for col, op, val in d['conds']:
            conds.append([col, op, de_tokenize(val)])
    ...
    def from_sequence(cls, sequence, table, lowercase=True):
        ...
        headers = [de_tokenize(h) for h in table['header']]
        ...
        try:
            agg_col = find_column(de_tokenize(flat))
        except Exception as e:
        ...
        while where_terms:
            ...
            try:
                cond_col = find_column(de_tokenize(col_tokens))
            except Exception as e:
            ...
            cond_val = de_tokenize(flatten(where_terms[:val_end_index]))
